...
ip_pool =["192.168.143.104", "192.168.143.74", "192.168.143.152", "192.168.143.100","192.168.143.212", "192.168.50.153"] #alexa, nestcam, fridge
...
def _get_fid(ip):
    ...
    try:
        ...
        fid = (src_ip, dst_ip, src_port, dst_port, flow_type_int)
        if src_ip in ip_pool:
            out_flow_map[fid] = 1
            fid = (src_ip, '1.1.1.1', 1111, 1111, 6) #outbound
        if dst_ip in ip_pool:
            in_flow_map[fid] = 1
            fid = ('1.1.1.1', dst_ip, 1111, 1111, 17) #inbound
        # print(fid)
        return fid
    except:
        return fid

def _get_direction(ip):
    ## code for figuring out upstream /downstream and assign dummy fid accordingly ##
    fid = ('', '', -1, -1, -1)
    return fid
    ...
    try:
        ...
        with open(pcap_file, 'rb') as f:
            ...
            for timestamp, buf in pcap:
                ...
                eth_hdr = eth.pack_hdr()
                if eth.type != ETH_TYPE_IP:
                    continue
                if (verbose > 3) and (i % 500000 == 0):
...
def _flows2subflows(flows, interval=10, *, flow_pkts_thres=2, verbose=1):
    ...
    #interval = _get_split_interval(flow_durations, q_interval=q_interval)
    ...
    for i, (fid, pkts) in enumerate(flows):
        ...
        for j, pkt in enumerate(pkts):
            ...
            if (fid[4] == 6):
                # handle TCP packets, TCP is 6
                # a timeout (the idle time) is the duration between the previous pkt and the current one.
                if pkt_time - _get_frame_time(subflow_tmp[-1]) > interval:
                    subflows.append((fid, subflow_tmp))
...
def _get_header_features(flows):
    ...
    def _parse_tcp_flgs(tcp_flgs):
        tcp_flgs_bin = f'{tcp_flgs:08b}'
        tcp_flgs_list = [int(x) for x in tcp_flgs_bin]
        return tcp_flgs_list
    ...
    for fid, pkts in flows:
        ...
        for i, pkt in enumerate(pkts):
            ip_hdr = dpkt.ip.IP(pkt[2])
            if ip_hdr.p == 6:  # tcp
                tcp_hdr = dpkt.tcp.TCP(pkt[3])
                flgs_lst += np.asarray(_parse_tcp_flgs(tcp_hdr.flags)).reshape(-1, 1)  # parses tcp.flgs
            header_features.append(ip_hdr.ttl)
        feature = list(flgs_lst.flatten()) + header_features
        features.append(feature)

    return features


def _get_num_flows(flows):
    num_flows_list = []
    for fid, pkts in flows:
        fid_map = {}
        for pkt in pkts:
            fid_map[pkt[4]] = 1
        num_flows = len(fid_map)
        num_flows_list.append(num_flows)
    return num_flows_list
...
def _get_SIZE(flows):
    ...
    fids = []
    for fid, pkts in flows:
        sizes = [dpkt.ip.IP(pkt[2]).len for pkt in pkts]
...
def _get_STATS(flows):
    ...
    for fid, pkts in flows:
        ...
        q1_size, q2_size, q3_size = np.quantile(sizes, q=[0.25, 0.5, 0.75])  # q should be [0,1] and q2 is np.median(data)
        q1_iat, q2_iat, q3_iat = np.quantile(iats, q=[0.25, 0.5, 0.75])  # q should be [0,1] and q2 is np.median(data)

        # print(len(out_flow_map), len(in_flow_map), fid[4])
        if fid[4] == 6:
            base_features = [sub_duration, pkts_rate, bytes_rate, np.mean(sizes), np.std(sizes),
                            q1_size, q2_size, q3_size, np.min(sizes), np.max(sizes), np.mean(iats), np.std(iats), q1_iat, q2_iat,
                            q3_iat, np.min(iats), np.max(iats), len(out_flow_map), 1]
        elif fid[4] == 17:
            base_features = [sub_duration, pkts_rate, bytes_rate, np.mean(sizes), np.std(sizes),
                            q1_size, q2_size, q3_size, np.min(sizes), np.max(sizes), np.mean(iats), np.std(iats), q1_iat, q2_iat,
                            q3_iat, np.min(iats), np.max(iats), len(in_flow_map), -1]
    ...
    if len(features) == 0:
        features == [[0]* 18 + [1], [0] * 18 + [-1]]
    elif len(features) == 1:
        if features[0][-1] == 1:
            features.append([0] * 18 + [-1])
        else:
            features.append([0] * 18 + [1])
...
def _get_SAMP(flows, sampling_feature='SAMP_NUM', sampling_rate=0.1, verbose=1):
    ...
    for fid, pkts in flows:
        ...
        each_flow_features = []
        pkt_times = [_get_frame_time(pkt) for pkt in pkts]
        pkt_sizes = [dpkt.ip.IP(pkt[2]).len for pkt in pkts]
        samp_sub = -1
...
class PCAP:
    ...
    def _pcap2flows(self, interval=0, q_interval=0.1, *, tcp_timeout=600, udp_timeout=600):
        ...
        flows = _pcap2flows(self.pcap_file, self.flow_pkts_thres, tcp_timeout=tcp_timeout, udp_timeout=udp_timeout,
        ...
        self.flows = _flows2subflows(flows, self.interval, flow_pkts_thres=self.flow_pkts_thres, verbose=self.verbose)
    ...
    @timing
    def _flow2features(self, feat_type='IAT', *, fft=False, header=False, dim=None, num_flow=True):
        ...
        num_pkts = [len(pkts[1]) for pkts in self.flows]
        if not dim:
        ...
        # append number of flows
        if num_flow:
            num_flows = _get_num_flows(self.flows)
            self.features = [f + [n] for f, n in zip(self.features, num_flows)]
            self.dim += 1
    ...
    def flow2features(self, feat_type='IAT', *, fft=False, header=False, dim=None, num_flow=False):
        """Extract features from each flow according to feat_type, fft and header.
        ...
        -------
            self
        """
        _, tot_time = self._flow2features(feat_type, fft=fft, header=header, dim=dim, num_flow=num_flow)
